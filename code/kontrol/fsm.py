...
"""
    Basic Pykka based finite state-machine implementation.
"""
...
class Aborted(Exception):
    """
    User exception that can be raised from within a state for clarity
    """
...
class FSM(ThreadingActor):
    ...
    def specialized(self, msg):
        ...
        if req == 'get':
            #
            # - default 'get' request
            # - proxy to getattr() on the actor
            #
            return getattr(self, msg['key'], None)
        elif req == 'set':
            
            #
            # - default 'set' request
            # - proxy to setattr() on the actor
            #
            setattr(self, msg['key'], msg['value'])
        elif req == 'shutdown':
